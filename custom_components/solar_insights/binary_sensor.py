...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            SunnyBinarySensor(hass, config_entry),
            SunnyMedianBinarySensor(hass, config_entry),
        ]
    )
...
class SunnyMedianBinarySensor(MedianWindowMixin, BasePanelEntity, BinarySensorEntity):
    """Binary sensor that is on when median incident-normalized irradiance is sunny."""

    _attr_device_class = BinarySensorDeviceClass.LIGHT
    _attr_icon = "mdi:weather-sunny"

    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry) -> None:
        """Initialize the median sunny binary sensor."""
        super().__init__(hass, config_entry)
        self._attr_translation_key = "sunny_median"
        self._attr_unique_id = f"{config_entry.entry_id}_sunny_median"
        self._attr_is_on = None

    def _update_state(self) -> None:
        """Fetch new state data for the sensor."""
        try:
            self._attr_is_on = self.is_sunny_median()
        except Exception as err:
            _LOGGER.error("Error updating %s: %s", self.name, err)
